...
* **Cookies 檔**（選用，用於避免 YouTube IP 封鎖與年齡限制）: 由使用者自瀏覽器匯出的 Netscape 格式 cookie 檔。
  > ⚠️ **絕對不要把這個檔案 commit 進版控。** 它含有你 Google 帳號的有效登入 session，取得的人可以直接以你的身分登入。已列入 `.gitignore`，建議放在 repo 之外。
* **FFmpeg**: `ffmpeg.exe` 與 `ffprobe.exe` 必須存在於專案根目錄或 PATH 中。這是 `yt-dlp` 執行字幕格式轉換 (`--convert-subs`) 的必要依賴。
