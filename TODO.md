...
- [ ] 將整個專案發展成完整前後端架構產品（Web 介面 / 儀表板 / 可能對外的產品）——結論是「先驗證再包裝」，Skill化是產品化的必經之路不是繞路。**網站/App 介面的細部規劃已於 2026-07-24 移至獨立資料夾 `Side Project/DebateSystem-Web/TODO_web_app.md`（比照 DebateSystem-TW 慣例，與本 repo 分開維護）**
- [ ] **⚠️ 待確認的核心系統變更（源自 DebateSystem-Web 開發需求）**：Web 端要做「圖表標點位」功能，需要 `persona_debates` 新增結構化價位欄位（如 `key_levels_json`）並修改 `trader-debate` SKILL.md prompt——這屬於辯論協議本體變更，需要 `preregistration.md` §8 增補登記＋`protocol_version` 升版才能動工，細節見 `DebateSystem-Web/TODO_web_app.md`「⚠️ 重大發現」一節
